...
int main(int argc, char **argv)
{
  ...
  int* message = (int*) malloc(2e6);

  int i = 0;

  if (rank == 0)
    for (i = 0; i < 2e6/sizeof(int) ; i++)
      message[i] = 0;
  ...
  {
    if (rank == 0) {
      MPI_Send(message, 2e6/sizeof(int), MPI_INT, destination, tag, MPI_COMM_WORLD);
      //printf("At loop %d, process %d send %d to process %d.\n", i, rank, message, destination);
      MPI_Recv(message, 2e6/sizeof(int), MPI_INT, origin, tag, MPI_COMM_WORLD, &status);
    }
    else {
      MPI_Recv(message, 2e6/sizeof(int), MPI_INT, origin, tag, MPI_COMM_WORLD, &status);
      MPI_Send(message, 2e6/sizeof(int), MPI_INT, destination, tag, MPI_COMM_WORLD);
